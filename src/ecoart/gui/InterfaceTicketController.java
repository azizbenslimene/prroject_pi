...
public class InterfaceTicketController implements Initializable {
   ...
   ImageView QRpath_a = new ImageView();
    ...
    public void genererPDF(String nom, String date, String lieu, int prix ,ImageView QRpath_a) {
        // Créez un document PDF avec iText
        ...
        if (fichierPDF != null) {
            try {
                // Créez le fichier PDF à partir de l'emplacement choisi
                ...
                String dateFormatee = formatDate.format(dateActuelle);
                document.add(new Paragraph("Date : " + dateFormatee));

                // Informations de la facture : nom, date, lieu et prix
                document.add(new Paragraph("Nom : " + nom));
                document.add(new Paragraph("Date : " + date));
                document.add(new Paragraph("Lieu : " + lieu));
                document.add(new Paragraph("Prix : " + prix));
                document.add(new Paragraph("QR code : " + QRpath_a));

                // Fermeture du document PDF
                document.close();

                // Imprimez le document PDF
                try {
                    Desktop.getDesktop().print(fichierPDF);
                } catch (IOException e) {
                    e.printStackTrace();
                }
